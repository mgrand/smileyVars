package com.markgrand.smileyVars.testUtil;
...
public class MockDatabaseMetadata implements DatabaseMetaData {
    ...
    private SQLException getDatabaseProductNameThrows;
     ...
     * <code>NULL</code> values will appear at the beginning. By contrast, the method
    ...
    public String getDatabaseProductName() throws SQLException {
        if (getDatabaseProductNameThrows != null) {
            throw getDatabaseProductNameThrows;
        }
        return databaseProductName;
    }

    public MockDatabaseMetadata databaseProductName(String value) {
        databaseProductName = value;
        return this;
    }

    public MockDatabaseMetadata getDatabaseProductNameThrows(SQLException exception) {
        getDatabaseProductNameThrows = exception;
        return this;
    }
    ...
    public MockDatabaseMetadata driverName(String value) {
        driverName = value;
        return this;
    }
     ...
     * The JDBC types are the generic SQL data types defined in <code>java.sql.Types</code>.
     *
     * @return <code>true</code> if so; <code>false</code> otherwise
     * @throws SQLException if a database access error occurs
     */
     ...
     * and <i>toType</i>.  The JDBC types are the generic SQL data types defined in <code>java.sql.Types</code>.
     *
     ...
     * parameter name criteria are returned.  They are ordered by PROCEDURE_CAT, PROCEDURE_SCHEM, PROCEDURE_NAME and
     * SPECIFIC_NAME. Within this, the return value, if any, is first. Next are the parameter descriptions in call
     ...
     * <LI> TRUNCATE (not enclosed in quotes)        - if the specified default value cannot be represented without
     * truncation
     ...
     * is returned if this row describes the procedure's return value.  For result set columns, it is the ordinal
     * position of the column in the result set starting from 1.  If there are multiple result sets, the column ordinal
     * positions are implementation defined.
     ...
     * catalog, schema, table name and type criteria are returned.  They are ordered by
     ...
     * SELF_REFERENCING_COL_NAME are created. Values are "SYSTEM", "USER", "DERIVED". (may be <code>null</code>)
     * </OL>
     *
    ...
    /**
     * Retrieves the schema names available in this database.  The results are ordered by <code>TABLE_CATALOG</code>
     * and
     ...
     * of 1 represents the first column of the primary key, a value of 2 would represent the second column within the
     * primary key).
     ...
     * of 1 represents the first column of the foreign key, a value of 2 would represent the second column within the
     * foreign key).
     ...
     * a single row with a TYPE_NAME of DISTINCT and a DATA_TYPE of Types.DISTINCT. If the database supports SQL
     * structured types, then getTypeInfo() will return a single row with a TYPE_NAME of STRUCT and a DATA_TYPE of
     * Types.STRUCT.
     *
     ...
     * <code>ResultSet.rowDeleted</code>.  If the method
     ...
     * name.  In this case, the catalog and schemaPattern parameters are ignored.
     *
     * <P>Each type description has the following columns:
     * <OL>
     ...
     * DISTINCT type or the type that implements the user-generated reference type of the SELF_REFERENCING_COLUMN of a
     * structured type as defined in java.sql.Types (<code>null</code> if DATA_TYPE is not DISTINCT or not STRUCT with
     ...
     * and table name are returned. The table name parameter may be a fully- qualified name, in which case, the catalog
     * and schemaPattern parameters are ignored. If a table does not have a super table, it is not listed here.
     * Supertables have to be defined in the same catalog and schema as the sub tables. Therefore, the type description
     * does not need to include this information for the supertable.
     *
     * <P>Each type description has the following columns:
     * <OL>
     ...
     * For a UDT, the type name is fully qualified. For a REF, the type name is fully qualified and represents the
     * target type of the reference type.
     * <LI><B>ATTR_SIZE</B> int {@code =>} column size.  For char or date
     * types this is the maximum number of characters; for numeric or decimal types this is precision.
     * <LI><B>DECIMAL_DIGITS</B> int {@code =>} the number of fractional digits. Null is returned for data types where
     * DECIMAL_DIGITS is not applicable.
     * <LI><B>NUM_PREC_RADIX</B> int {@code =>} Radix (typically either 10 or 2)
    ...
    /**
     * Retrieves whether a <code>SQLException</code> while autoCommit is <code>true</code> indicates that all open
     ...
     * this function within its schema.  This is a user specified, or DBMS generated, name that may be different then
     * the <code>FUNCTION_NAME</code> for example with overload functions
     * </OL>
     * <p>
     ...
     * if any, is first. Next are the parameter descriptions in call order. The column descriptions follow in column
     * number order.
     *
     ...
     * and schema. Pseudo or hidden columns may not always be stored within a table and are not visible in a ResultSet
     * unless they are specified in the query's outermost SELECT list. Pseudo or hidden columns may not necessarily be
     * able to be modified. If there are no pseudo or hidden columns, an empty ResultSet is returned.
     *
     * <P>Only column descriptions matching the catalog, schema, table
     * and column name criteria are returned.  They are ordered by
     ...
     * based on the column(s) for the auto generated key. Consult your JDBC driver documentation for additional
     * details.
     *
     * @return <code>true</code> if so; <code>false</code> otherwise
     * @throws SQLException if a database access error occurs
