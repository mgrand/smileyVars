 ...
 * @author Mark Grand
 */
class NReaderValue extends AbstactPreparedStatementValue {
     ...
     */
    NReaderValue(Reader reader, int length) {
        this.reader = reader;
     ...
     * @param length The value that this object will be used to set in a prepared statement.
     */
    NReaderValue(Reader reader, long length) {
        this.reader = reader;
        this.length = length;
    }
    ...
    void setParameter(PreparedStatement pstmt, int i) throws SQLException {
        if (length == null) {
            pstmt.setNCharacterStream(i, reader);
        } else {
            pstmt.setNCharacterStream(i, reader, length);
        }
    }
}
